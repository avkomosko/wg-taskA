let input = '1,2,5,7,8,3,15,4,19,13,14,25,35,9,17,14,16,37,20,36,153,152,151';
...
function solve(input) {
  ...
  function findSequence(arr) {
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      if (length >= 3) {
        if (stack[stack.length - 1] !== length || sequence[i-1] !== sequence[i]) {
          stack.push(length);
          map.set(i, length);
        }
      }
      console.log(sequence, stack, map);
    }
    return map;
  }
